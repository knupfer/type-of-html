...
main = withSystemTempDirectory "compile" $ \tmp -> mainWith $ do
  ...
  f "listOfListOf"         (\x -> renderByteString $ div_ [i_ [span_ x]]) ()      $ ghc [ 1200             ]
  f "helloWorld"           (renderByteString . M.helloWorld) ()                   $ ghc [ 1248,    0,   16 ]
  f "page"                 (renderByteString . M.page) ()                         $ ghc [ 1256             ]
  f "table"                (renderByteString . M.table) (2,2)                     $ ghc [ 2480,    0,   96 ]
  f "AttrShort"            (renderByteString . M.attrShort) ()                    $ ghc [ 3448,   72       ]
  f "pageA"                (renderByteString . M.pageA) ()                        $ ghc [ 2608, -216       ]
  f "AttrLong"             (renderByteString . M.attrLong) ()                     $ ghc [ 3448,   72, -216 ]
  f "Big table"            (renderByteString . M.table) (15,15)                   $ ghc [48136,    0, 5424 ]
  f "Big page"             (renderByteString . B.page) ()                         $ ghc [27592,   -8,  184 ]
  let g x y z = validateAction x (compile tmp) y . allocsError 7 z                $ ghc [  120,    0,    2 ]
  ...
  g "Compile X1.hs"     "bench/Compilation/X1.hs"                                 $ ghc [    4             ]
  ...
  g "Compile X64.hs"    "bench/Compilation/X64.hs"                                $ ghc [  203,    2,   -1 ]
